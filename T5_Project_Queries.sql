...
-- Query 2
select * from t5_customer c where c.T5_CUST_ID in 
(select a.T5_CUST_ID from t5_customer a inner join t5_premium_payment b on a.T5_CUST_ID = b.T5_CUST_ID 
where b.T5_PREMIUM_PAYMENT_AMOUNT > (select sum(t5_cust_id) from t5_customer));

##Query 3
select * from t5_insurance_company where 
(select count(*) from t5_product where t5_product.t5_company_name = t5_insurance_company.t5_company_name) 
>
(select count(*) from (select count(*) from t5_office where t5_office.t5_company_name=t5_insurance_company.t5_company_name group by t5_office.t5_department_name
...
-- Query 5 
select * from t5_vehicle where t5_vehicle_id in 
(select c.t5_vehicle_id from (select t5_vehicle_id, t5_vehicle_number, return_premium_amount(t5_policy_id) as b from t5_vehicle a)c 
where (c.b > c.t5_vehicle_number));

-- Query 6
select * from t5_customer where t5_cust_id in (select d.t5_cust_id from (select t5_cust_id, 
